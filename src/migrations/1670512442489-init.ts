...
export class init1670512442489 implements MigrationInterface {
    name = 'init1670512442489'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`owners\` (\`userId\` int NOT NULL AUTO_INCREMENT, \`walletAddress\` varchar(255) NOT NULL, \`isFraud\` tinyint NOT NULL, \`username\` varchar(255) NOT NULL, PRIMARY KEY (\`userId\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`campaigns\` (\`id\` varchar(255) NOT NULL, \`name\` varchar(255) NOT NULL, \`description\` varchar(255) NOT NULL, \`goal\` double NOT NULL, \`expires_at\` date NOT NULL, \`status\` enum ('ACTIVE', 'SUCCESSFUL', 'EXPIRED', 'FRAUD') NOT NULL, \`currency\` varchar(255) NOT NULL, \`ownerId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
